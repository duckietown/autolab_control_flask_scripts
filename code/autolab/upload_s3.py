    ...
    for rpath, realfile in toupload.items():
        ...
        try:
            ...
            if not_found:
                status = 'uploading'
                print('%15s %8s  %s' % (status, size, rpath))
                aws_object.upload_file(realfile, ExtraArgs={
                                       'ContentType': mime_type})

            else:
                raise
        url = 'http://%s.s3.amazonaws.com/%s' % (bucket_name, object_key)
        storage = dict(s3=dict(object_key=object_key,
                               bucket_name=bucket_name, url=url))
        uploaded.append(dict(size=size, mime_type=mime_type,
                             rpath=rpath, sha256hex=sha256hex, storage=storage))
    return uploaded
...
def upload_files(wd, aws_config, ignore_patterns=('.DS_Store',)):
    ...
    if not aws_config:
        ...
        return list(upload(aws_config, toupload))
